...
class ServiceProvider extends \Illuminate\Translation\TranslationServiceProvider {
	...
	protected $commands = [
        ...
        'Hpolthof\Translation\Console\Commands\RemoveCommand',
	];
	...
	public function boot()
	{
		...
		if($this->app['config']->get('translation-db.api')) {
			$routeConfig = [
	...
	protected function registerDatabase()
	{
		...
		{
			return new DatabaseLoader($app, ServiceProvider::getDomainId($app));
		});
	}

    public static function getDomainId($app) {
        $id = $app['config']->get('translation-db.get_domain');
        return (is_int($id)) ? $id : $id(); //either integer or function...
    }
